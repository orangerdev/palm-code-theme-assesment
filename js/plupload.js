	...
	plupload.Uploader = function (settings) {
		...
		function calc() {
			...
			if (total.size === undef) {
				total.percent =
					files.length > 0
						? Math.ceil((total.uploaded / files.length) * 100)
						: 0;
			} else {
				...
				total.percent =
					total.size > 0 ? Math.ceil((total.loaded / total.size) * 100) : 0;
			}
		}
			...
			init: function () {
				...
				self.bind("FilesAdded", function (up, selected_files) {
					...
					if (filters && filters.length) {
						...
						extensionsRegExp = new RegExp(
							extensionsRegExp.join("|") + "$",
							"i",
						);
					}
				...
				self.bind("UploadProgress", function (up, file) {
					file.percent =
						file.size > 0 ? Math.ceil((file.loaded / file.size) * 100) : 100;
					calc();
				});
